...
| 日期       | 变更内容                                                       |
|------------|----------------------------------------------------------------|
| 2026-09-19 | 新增"下载与安装"章节：SmartScreen 首次运行拦截的缘由与绕过方式。 |

## 下载与安装

从 [Gitee Release](https://gitee.com/mwcxlinxun/deep-seek-otter/releases)（国内，推荐）或 [GitHub Release](https://github.com/linxunxr/DeepSeek-Otter/releases) 下载 `DeepSeek.Otter_<版本>_x64-setup.exe`，双击安装。装完启动即用，无需 Node.js、无需联网（dsh 依赖树已离线打进安装包）。

**首次运行弹出"Windows 已保护你的电脑"？** 安装包未购买商业代码签名，Microsoft Defender SmartScreen 对无签名的新程序默认拦截。点击 **"更多信息" → "仍要运行"** 即可继续安装，不影响使用；随版本发布与下载量积累，该提示会逐渐不再出现。已安装用户走应用内更新（托盘"检查更新…"）不经过此检查。
...
- **强绑定离线打包**：安装包 76 MB（node + dsh 完整依赖树），首装离线可用；
